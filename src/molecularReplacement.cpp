...
Real quantifyWeightedMatch(XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	...
	if (params.RFactorWeight!=0) 
	{
		...
		//cout <<"RFactor took "<<calcTimeDiff(start, end)<<endl;
		//cout <<"RFactor= "<<RFactorFast(xray.i, expXRay.F)<<" weight= "<<params.RFactorWeight<<endl;
		if (isnan(score))
		{
	...
	if (params.DiffOfSquaresWeight!=0) 
	{
		...
		//gettimeofday(&end, NULL);
		//cout <<"SqrDiff took "<<calcTimeDiff(start, end)<<endl;
		//cout <<"score= "<<score<<" DiffOfSquares= "<<params.DiffOfSquaresWeight<<endl;
		//score+=sqrDiff/Real(npoint);
		//cout <<"sqrDiff= "<<sqrDiff/Real(npoint)<<endl;
		if (isnan(score))
		{
			error("score= nan", __LINE__, __FILE__);
		}
	...
	if (params.PearsonWeight!=0) 
	{
		...
		//cout <<"pearson= "<<(1.0-calcPearson(xray.i, expXRay.i))<<" ProductWeight= "<<params.PearsonWeight<<endl;
		if (isnan(score))
		{
			error("score= nan", __LINE__, __FILE__);
		}
	...
	if (params.ProductWeight!=0) 
	{
		...
		//cout <<"score= "<<score<<" ProductWeight= "<<params.ProductWeight<<endl;
		if (isnan(score))
		{
			error("score= nan", __LINE__, __FILE__);
		}
	...
	{
		//gettimeofday(&start, NULL);
		cout <<"Before calcMLRF0"<<endl;
		score+=calcMLRF0(xray, expXRay)*params.MLRF0Weight;
		//gettimeofday(&end, NULL);
		//cout <<"MLRF0 took "<<calcTimeDiff(start, end)<<endl;
		//cout <<"score= "<<score<<" MLRF0Weight= "<<params.MLRF0Weight<<endl;
		if (isnan(score))
		{
			error("score= nan", __LINE__, __FILE__);
		}
	...
	if (params.RotationWeight!=0)
	{
		...
		//gettimeofday(&end, NULL);
		//cout <<"RPS took "<<calcTimeDiff(start, end)<<endl;
		if (isnan(score))
		{
			error("score= nan", __LINE__, __FILE__);
		}
	...
	if (params.LogSquareDiffWeight!=0)
	{
		...
		//gettimeofday(&end, NULL);
		//cout <<"LogSqrDiff took "<<calcTimeDiff(start, end)<<endl;
		if (isnan(score))
		{
			error("score= nan", __LINE__, __FILE__);
		}
...
Real phase(vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	cout <<endl<<"Initial RFactor= "<<initRFactor<<endl;
	cout <<"Final RFactor= "<<finalRFactor<<endl;
	return finalRFactor;
}
...
Real calcFitness(Vector &degOfFreedom, vector<ProteinStruct> Proteins, XRayStruct &XRay, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	if (params.OptimizeOccupancies)
	{
		...
		{
			setSegmentOccupancies(Proteins[0], degOfFreedom, params);
		}
		else 
		{
			setSegmentOccupancies(Proteins[0], degOfFreedom, 1);
...
Real calcFitnessChange(Vector &degOfFreedom, ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params, int pick)
{
	...
	//gettimeofday(&end, NULL);
	//cout <<"calcXRayScatteringChange took "<<calcTimeDiff(start, end)<<endl;
	int nXrayProt=xray.Proteins.size();
	if (params.SegmentsFile!="")
	{
		for (int i=0;i<nXrayProt;i++)
		{
			setSegmentOccupancies(xray.Proteins[i], degOfFreedom);
		}
	}
	if (params.CorrectionFile!="") correctIntensity(xray);
	if (params.OccupancyCorrectionFile!="" && params.OptimizeOccupancies) correctOccupancyIntensity(xray, Protein, params);
	...
	if (params.ClashScoreUse=="DuringMR")
	{
		...
		//Real clashScoreSlow;
		calcClashScoreFast(degOfFreedom, xray.Proteins, params, clashCC, clashCS, clashSS);
...
*/
		//cout <<"clashScore= "<<clashScoreSlow<<" clashScoreFast= "<<clashScore<<endl;
	...
	if (params.OptimizeOccupancies)
	{
		if (params.SegmentsFile!="")
		{
			aveOccupancy=calcAveOccupancy(xray.Proteins[0]);
		}
		else aveOccupancy=calcAveOccupancy(degOfFreedom, params);
		score+=(1.0-aveOccupancy)*params.OccupancyWeight;
	}
...
void makeUnitCellAndImagesFast2(vector<ProteinStruct> &Proteins, vector<ProteinStruct> &allProteins)
{
	...
	gettimeofday(&end, NULL);
	cout <<"reserve took "<<calcTimeDiff(start, end)<<endl;
	gettimeofday(&start, NULL);
	for (int i=0;i<nprot;i++)
	{
		SafeAlloc(Proteins[i].center, 3, "center");
	...
	gettimeofday(&end, NULL);
	cout <<"symmetry took "<<calcTimeDiff(start, end)<<endl;
	gettimeofday(&start, NULL);
	...
	gettimeofday(&end, NULL);
	cout <<"image took "<<calcTimeDiff(start, end)<<endl;
}
...
Real calcBFactorFitness(Vector bFactorScale, MRStruct args)
{
	...
	cout <<"bFactorScale= "<<bFactorScale[0]<<" score= "<<score<<endl;	
	return score;
}
...
Real calcFormFactorFitness(Vector formFactorScale, MRStruct args)
{
	...
	score=calcScattering(args.Proteins, args.xray, args.expXRay, args.params);
	cout <<"formFactorScale= "<<formFactorScale[0]<<" score= "<<score<<endl;	
	return score;
}
...
Real MRRotationalGridSearch(Vector &bestGrid, Real rinc, int npoint, ProteinStruct Protein, XRayStruct xray, XRayStruct &expXRay, LatticeStruct lattice, XRayParamStruct &params, vector<Array3D> &degOfFreedoms, Array3D &scores)
{
	...
	cout <<"In MRGridSearch"<<endl;
	cout <<"bestGrid.size()= "<<bestGrid.size()<<endl;	
	...
	for (int i=0;i<npoint;i++)
	{
		...
		cout <<"rx= "<<degOfFreedom[X]<<" rmin= "<<rmin<<endl;
	...
	cout <<"rmin= "<<rmin<<endl;
	cout <<"Leaving MRGridSearch"<<endl;
	return rmin;
}
...
Real minimizeRotation(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &degOfFreedom, Real maxHours)
{
	...
	MRStruct args;
	Real (*func_to_minimize)(Vector, MRStruct)=calcFitness;
	cout <<"In minimizeRotation memory= "<<memoryUsage()<<endl;
	args.Protein=Protein;
	args.xray=xray;
	args.expXRay=expXRay;
	args.params=params;

	cout <<"Before conjugateGradient memory= "<<memoryUsage()<<endl;
	...
	cout <<"After conjugateGradient memory= "<<memoryUsage()<<endl;
...
Real minimizeRotationOnly(ProteinStruct Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &degOfFreedom, Real maxHours)
{
	...
	gettimeofday(&end, NULL);
	cout <<"minimizeRotationOnly took "<<calcTimeDiff(start, end)<<endl;
...
bool minimizeRotationUsingEstimatedScore(ProteinStruct Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &degOfFreedom)
{
	...
	cout <<"score= "<<score<<" score_old= "<<scoreOld<<endl;
	if (score>scoreOld)
	{
		degOfFreedom=backUpDOF;
		cout <<"Minimization using estimated score failed"<<endl;
		return false;
	}
	else
	{
		cout <<"Minimization using estimated score worked"<<endl;
		return true;
	}
}
...
void minimizeRotationFast(ProteinStruct Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &degOfFreedom)
{
	...
	cout <<"minimizeRotationFast took "<<calcTimeDiff(start, end)<<endl;
}
...
Real minimizeRotationAnalytical(vector<Real> &coefficient, MRStruct args, Real dScoreCriteria, Real maxHours)
{
	...
	cout <<"Before score_old"<<endl;
	score_old=func_to_minimize(coefficient, args);
	cout <<"score_old= "<<score_old<<endl;
	timeval start, end;
	...
	while (true)
	{
		...
		cout <<"time= "<<calcTimeDiff(start, end)/3600.0<<endl;
...
void getBestResults(Matrix &localMinima, Vector &localMinimaScores)
{
	...
	for (int i=0;i<Size1;i++)
	{
		cout <<"score= "<<localMinimaScores[i]<<" cutoff= "<<ave*(1.0-w)+best*w<<endl;
...
Real MRRotate(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params, Matrix &localMinima, Vector &degOfFreedom)
{
	...
	for (int i=0;i<3;i++)
	{
		cout <<"degOfFreedom["<<i<<"]= "<<degOfFreedom[i]<<endl;
	}
...
Real MRTranslationGridSearch(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &dPos)
{
	...
	cout <<"bestScore= "<<bestScore<<" oldScore= "<<oldScore<<endl;
	bestDPos=dPos;
	for (int i=0;i<steps;i++)
	{
		dPos[X]=xinc*Real(i);
		cout <<"dPos[X]= "<<dPos[X]<<" bestScore= "<<bestScore<<endl;
		for (int j=0;j<steps;j++)
		{
			...
			for (int k=0;k<steps;k++)
			{
				...
				if (score<bestScore)
				{
					...
					cout <<"dPos[X]= "<<dPos[X]<<endl;
					cout <<"dPos[Y]= "<<dPos[Y]<<endl;
					cout <<"dPos[Z]= "<<dPos[Z]<<endl<<endl;
				}
			}
		}
	}
	dPos=bestDPos;
	gettimeofday(&end, NULL);
	cout <<"MRTranslationGridSearch took "<<calcTimeDiff(start, end)<<endl;
	return bestScore;
}
...
Real minimizeTranslationOnly(vector<ProteinStruct> Proteins, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &degOfFreedom)
{
	...
	/*
	   ...
	   {
	   cout <<"i= "<<i<<" int= "<<xray.i[i]<<" real= "<<xray.real[i]<<" imag= "<<xray.imag[i]<<" F= "<<xray.F[i]<<endl;
	   }
	 */
	...
	gettimeofday(&end, NULL);
	cout <<"minimizeTranslationOnly took "<<calcTimeDiff(start, end)<<endl;
...
Real minimizeRotationTranslation(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Vector &degOfFreedom)
{
	...
	MRStruct args;
	Real (*func_to_minimize)(Vector, MRStruct)=calcRotationTranslationFitness;
	cout <<"In rotationTranslationMinimization memory= "<<memoryUsage()<<endl;
	args.Protein=Protein;
	args.xray=xray;
	args.expXRay=expXRay;
	args.params=params;
	SafePushBack(args.Proteins, Protein, "Protein");
	cout <<"Before nonlinearConjugateGradient memory= "<<memoryUsage()<<endl;
	score=conjugateGradient(degOfFreedom, args, func_to_minimize, dScoreCriteria, maxHours);
	cout <<"After nonlinearConjugateGradient memory= "<<memoryUsage()<<endl;
...
Real MRTranslate(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params, Matrix &degOfFreedoms, Vector &bestDegOfFreedom, Vector &bestDPos)
{
	...
	cout <<"There are "<<ncandidates<<" orientations to try."<<endl;
	...
	printReplica(degOfFreedom, Proteins, outputFile, params);
...
Real MRRotateTranslate(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	cout <<"After MRTranslate or minimizeRotation"<<endl;
	...
	cout <<"Before calcScattering"<<__LINE__<<endl;
	SafePushBack(Proteins, Protein, "Proteins");
...
void printDofs(Matrix &degOfFreedoms)
{
	...
	for (int i=0;i<nrep;i++)
	{
		...
		{
			cout <<degOfFreedoms[i][j]<<"\t";
		}
		cout <<endl;
	}
}

void getSizeOfDegOfFreedoms(string degOfFreedomFile, int &nrep, int &ndegOfFreedom)
{
	vector<string> lines;
	int nlines, degOfFreedom, rep;
	...
	for (int i=0;i<nlines;i++)
	{
		...
		if (rep>nrep) nrep=rep;
		if (degOfFreedom>ndegOfFreedom) ndegOfFreedom=degOfFreedom;
	}
	nrep++;
	ndegOfFreedom++;
}
...
void initializeDegOfFreedoms(Matrix &degOfFreedoms, string degOfFreedomFile)
{
	...
	getSizeOfDegOfFreedoms(degOfFreedomFile, nrep, ndegOfFreedom);
	Safe2DAlloc(degOfFreedoms, nrep, ndegOfFreedom, "degOfFreedoms");
...
void makeRandomChange(Vector &coefficient, MoveStruct &move, int &pick, Real xlength, Real ylength, Real zlength, string spaceGroup, int nprot)
{
	...
	//while (pick<3) pick=pickRandomDOF(move, spaceGroup, nprot);
	...
	//cout <<"moveSizeProb= "<<moveSizeProb<<" moveSize= "<<moveSize<<" move.width["<<pick<<"]= "<<move.width[pick]<<endl;
...
*/
	...
	//for (int prot=0;prot<nprot;prot++)
	//{
	//	cout <<"Before calcFitnessChange"<<endl;
	//	score=calcFitnessChange(degOfFreedoms, Proteins[prot], xray, expXRay, lattice, params, prot*6);
	//	cout <<"After calcFitnessChange"<<endl;
	//}
	//calcTotalFormFactor(xray);
	//calcIntensity(xray);
	//score=calcRFactor(xray, expXRay, params);
...
void calcScores(Matrix &degOfFreedoms, Vector &score, Matrix &scores, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	cout <<"In calcScores"<<endl;
	for (int i=0;i<nrep;i++)
	{
		cout <<"degOfFreedoms:"<<endl;
		printVector(degOfFreedoms[i]);
...
/*
	...
	for (int i=0;i<nprot;i++)
	{
		for (int j=0;j<nsym;j++)
		{
			for (int k=0;k<npoint;k++)
			{
				cout <<"i= "<<i<<" j= "<<j<<" k= "<<k<<" slowReal= "<<slowXRay.complexSymReal[i][j][k]<<" fastReal= "<<xray.complexSymReal[i][j][k]<<" slowImag= "<<slowXRay.complexSymImag[i][j][k]<<" fastImag= "<<xray.complexSymImag[i][j][k]<<endl;
			}
		}
	}
	endProgram(__LINE__, __FILE__);
*/
	...
	for (int i=0;i<nsteps;i++)
	{
		...
		//scoreSlow=calcFitness(degOfFreedom, Proteins, slowXRay, expXRay, lattice, params);
		//params.UseFastTranslationRotation=true;
		//cout <<"pick= "<<pick<<"\t"<<score<<"\t"<<scoreSlow<<endl;
		//cout <<endl;
		//printVector(degOfFreedom);
		//cout <<endl;
		gettimeofday(&end, NULL);
		...
		dScore=score-score_old;
		//cout <<"score= "<<score<<" dScore= "<<dScore<<" pick= "<<pick<<endl;
	...
	cout <<"score_initial= "<<score_initial<<" score_final= "<<score_old
...
/*
	cout <<"score_initial= "<<score_initial<<endl;
	params.UseFastTranslationRotation=false;
	cout <<"Before calcFitness"<<endl;
	scoreSlow=calcFitness(degOfFreedom, Proteins, slowXRay, expXRay, lattice, params);
	params.UseFastTranslationRotation=true;
	cout <<"score_initial= "<<score_initial<<" scoreSlow= "<<scoreSlow<<endl;
	int nsym, npoint;
	Get3DVectorSize(xray.complexSymTempReal, nprot, nsym, npoint);
	cout <<"nprot= "<<nprot<<" nsym= "<<nsym<<" npoint= "<<npoint<<endl;
	cout <<"degOfFreedom:"<<endl;
	printVector(degOfFreedom);
*/
/*
	for (int n=0;n<nprot;n++)
	{
		for (int j=0;j<nsym;j++)
		{
			for (int k=0;k<npoint;k++)
			{
				cout <<"real= "<<xray.complexSymTempReal[n][j][k]<<" realSlow= "<<slowXRay.complexSymTempReal[n][j][k]
					<<" imag= "<<xray.complexSymTempImag[n][j][k]<<" imagslow= "<<slowXRay.complexSymTempImag[n][j][k]
				//cout <<"real= "<<xray.complexSymTempReal[n][j][k]
	...
	{
		cout <<"n= "<<n<<" xray.i= "<<xray.i[n]<<" expXRay.i= "<<expXRay.i[n]<<endl;
	}
	endProgram(__LINE__, __FILE__);
*/
	/*
	   ...
	   for (int i=0;i<npoint;i++)
	   {
	   cout <<"realFast= "<<xray.real[i]<<" realSlow= "<<xraySlow.real[i]<<" imagFast= "<<xray.imag[i]<<" imagSlow= "<<xraySlow.imag[i]<<" h= "<<xray.miller[i].Pos[X]<<" k= "<<xray.miller[i].Pos[Y]<<" l= "<<xray.miller[i].Pos[Z]<<endl;
	   }
	   cout <<"score_initial= "<<score_initial<<"\t"<<" scoreSlow= "<<scoreSlow<<endl;
	 */
	score=score_initial;
	...
	for (int i=0;i<nsteps;i++)
	{
		...
		else
		{
			...
			if (params.OptimizeOccupancies && pick>=nprot*6)
			{
				//sumScatteringContinuousSegments(xray, degOfFreedom, nprot*6);
				//sumScatteringContinuousSegments(xray, Proteins[0], degOfFreedom, nprot*6);
				//cout <<"Before calcScatteringContinousSegmentsChange"<<endl;
				calcScatteringContinuousSegmentsChange(xray, Proteins[0], degOfFreedom, nprot*6, pick);
				//cout <<"After calcScatteringContinousSegmentsChange"<<endl;
			}
		}
		nmove=move.score.size();
		if (nmove%updateMoveSizeInterval==0 && i!=nmove)
		{
			...
			updateMoveProbabilities(move, nMoveTypes);
			//printMoveSizes(move);
			//printMoveProbabilities(move);
		}
	}
	acceptRate=calcAcceptRate(move);
	if (params.Verbose) 
	{
		cout <<"score_initial= "<<score_initial
...
void printReplica(Vector &degOfFreedom, vector<ProteinStruct> Proteins, string outputFile, XRayParamStruct &params)
{
	...
	if (params.OptimizeOccupancies)
	{
		...
		else setSegmentOccupancies(Proteins[0], degOfFreedom, 1);
	}
...
void printReplicas(Matrix &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayParamStruct &params, string replicaStructuresOutput)
{
	...
	for (int i=0;i<Size;i++)
	{
		...
		printReplica(degOfFreedoms[i], Proteins, outputFile, params);
	}
}
...
void printScores(Vector &degOfFreedoms, vector<ProteinStruct> Proteins, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	...
	calcClashScore(degOfFreedoms, Proteins[0], params, clashCC, clashCS, clashSS);
	//clashScore=calcClashScore(degOfFreedoms, Proteins[0], params, clashCC, clashCS, clashSS);
	//calcClashScore(degOfFreedoms, Proteins, params, clashCC, clashCS, clashSS);
	...
	if (params.CorrectionFile!="") correctIntensity(xray);
	if (params.OccupancyCorrectionFile!="" && params.OptimizeOccupancies) correctOccupancyIntensity(xray, degOfFreedoms, params);
	calcAmplitude(xray);
	...
	scoreIndex=xray.scoreIndex[SQR_DIFF];
	total=diffOfSqrs*params.DiffOfSquaresWeight/xray.stdDev[scoreIndex];
	scoreIndex=xray.scoreIndex[RFACTOR];
	total+=r*params.RFactorWeight/xray.stdDev[scoreIndex];
	scoreIndex=xray.scoreIndex[PEARSON];
	total+=pearson*params.PearsonWeight/xray.stdDev[PEARSON];
	scoreIndex=xray.scoreIndex[PRODUCT];
	total+=product*params.ProductWeight/xray.stdDev[PRODUCT];
	//total+=rotationPossibleScore*params.RotationWeight;
	scoreIndex=xray.scoreIndex[CLASH_CC];
	total+=clashCC*params.ClashWeightCoreCore/xray.stdDev[scoreIndex];

	scoreIndex=xray.scoreIndex[CLASH_CS];
	total+=clashCS*params.ClashWeightCoreSurface/xray.stdDev[scoreIndex];

	scoreIndex=xray.scoreIndex[CLASH_SS];
	total+=clashSS*params.ClashWeightSurfaceSurface/xray.stdDev[scoreIndex];

	cout <<"params.DiffOfSquaresWeight= "<<params.DiffOfSquaresWeight<<endl;
	cout <<"params.RFactorWeight= "<<params.RFactorWeight<<endl;
	cout <<"params.PearsonWeight= "<<params.PearsonWeight<<endl;
	cout <<"params.ProductWeight= "<<params.ProductWeight<<endl;
	cout <<"params.RotationWeight= "<<params.RotationWeight<<endl;

	cout <<"RFactor= "<<r<<endl;
	cout <<"DiffOfSquares= "<<diffOfSqrs<<endl;
	cout <<"Pearson= "<<pearson<<endl;
	cout <<"Product= "<<product<<endl;
	cout <<"Rotation= "<<rotationPossibleScore<<endl;
	cout <<"MRScore= "<<total<<endl<<endl;
}
...
void printScores(Matrix &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	...
	for (int i=0;i<nrep;i++)
	{
		cout <<"Scores for replica "<<i<<endl;
		printScores(degOfFreedoms[i], Proteins, xray, expXRay, params);
	}
}

void calcAccurateScore(Vector &degOfFreedoms, vector<ProteinStruct> Proteins, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	//Prints all of the components of XRayScore for a replica.
	string suffix, base, name;
	int nprot=Proteins.size();
	int scoreIndex;
	Real total;
	Real clashCC, clashCS, clashSS;
	Real r, diffOfSqrs, pearson;
	//Real product;
	//Real rotationPossibleScore;
	ProteinStruct Complex;
	vector<ProteinStruct> OriginalProteins;
	...
	if (params.OptimizeOccupancies)
	{
		if (params.SegmentsFile!="")
		{
			setSegmentOccupancies(Proteins[0], degOfFreedoms, params);
		}
		else 
		{
			setSegmentOccupancies(Proteins[0], degOfFreedoms, 1);
		}
	}
	OriginalProteins=Proteins;
	printClashScore(degOfFreedoms, Proteins[0], params);
	...
	scoreIndex=xray.scoreIndex[SQR_DIFF];
	total=diffOfSqrs*params.DiffOfSquaresWeight/xray.stdDev[scoreIndex];
	xray.score[scoreIndex]=diffOfSqrs;
...
Real calcScore(XRayStruct &xray, XRayParamStruct &params)
{
	int scoreIndex;
	Real total=0;
	Real clashCC, clashCS, clashSS;
	...
	scoreIndex=xray.scoreIndex[SQR_DIFF];
	diffOfSqrs=xray.score[scoreIndex];
	total=diffOfSqrs*params.DiffOfSquaresWeight/xray.stdDev[scoreIndex];
	cout <<"diffOfSqrs= "<<diffOfSqrs<<" DiffOfSquaresWeight= "<<params.DiffOfSquaresWeight<<" stdDev= "<<xray.stdDev[scoreIndex]<<endl;

	scoreIndex=xray.scoreIndex[RFACTOR];
	r=xray.score[scoreIndex];
	total+=r*params.RFactorWeight/xray.stdDev[scoreIndex];
	cout <<"r= "<<r<<" RFactorWeight= "<<params.RFactorWeight<<" stdDev= "<<xray.stdDev[scoreIndex]<<endl;

	scoreIndex=xray.scoreIndex[PEARSON];
	pearson=xray.score[scoreIndex];
	total+=pearson*params.PearsonWeight/xray.stdDev[PEARSON];
	cout <<"pearson= "<<pearson<<" PearsonWeight= "<<params.PearsonWeight<<" stdDev= "<<xray.stdDev[scoreIndex]<<endl;

	//scoreIndex=xray.scoreIndex[PRODUCT];
	//product=xray.score[scoreIndex];
	//total+=product*params.ProductWeight/xray.stdDev[PRODUCT];

	scoreIndex=xray.scoreIndex[CLASH_CC];
	clashCC=xray.score[scoreIndex];
	total+=clashCC*params.ClashWeightCoreCore/xray.stdDev[scoreIndex];
	cout <<"clashCC= "<<clashCC<<" ClashCCWeight= "<<params.ClashWeightCoreCore<<" stdDev= "<<xray.stdDev[scoreIndex]<<endl;

	scoreIndex=xray.scoreIndex[CLASH_CS];
	clashCS=xray.score[scoreIndex];
	total+=clashCS*params.ClashWeightCoreSurface/xray.stdDev[scoreIndex];
	cout <<"clashCS= "<<clashCS<<" ClashCSWeight= "<<params.ClashWeightCoreSurface<<" stdDev= "<<xray.stdDev[scoreIndex]<<endl;

	scoreIndex=xray.scoreIndex[CLASH_SS];
	clashSS=xray.score[scoreIndex];
	total+=clashSS*params.ClashWeightSurfaceSurface/xray.stdDev[scoreIndex];
	cout <<"clashSS= "<<clashSS<<" ClashSSWeight= "<<params.ClashWeightSurfaceSurface<<" stdDev= "<<xray.stdDev[scoreIndex]<<endl;
...
void calcAccurateScores(Matrix &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, Matrix &scores, Vector &xscores, XRayParamStruct &params)
{
	int nrep=degOfFreedoms.size();
	Real xscore;
	...
	{
		cout <<"i= "<<i<<endl;
		calcAccurateScore(degOfFreedoms[i], Proteins, xray, expXRay, params);
		SafePushBack(scores, xray.score, "scores");
		xscore=calcScore(xray, params);
		SafePushBack(xscores, xscore, "xscores");
	}
}


void printScoresToFile(Matrix &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	//Prints all of the components of XRayScore for all of the replicas.
	ofstream file;
	string outputFile, suffix;
	string base, name;
	int nrep=degOfFreedoms.size();
	int scoreIndex;
	Real clashCC, clashCS, clashSS;
	Real sqrOfDiff, r, pearson;
	Vector xscores;
	Matrix scores;
	...
	if (params.ClashScoreUse=="DuringMR") suffix="_Clash";
	if (params.OptimizeOccupancies) suffix+="_OptimizeOccupancies";	
	outputFile=removeExtension(params.ScoreFileBase)+suffix+".txt";
	OpenFileForWriting(outputFile, file);
	base=getBase(params.ReplicaStructuresOutput);

	calcAccurateScores(degOfFreedoms, Proteins, xray, expXRay, scores, xscores, params);

	file <<"SquareOfDifferences\tRFactor\tPearson\tClashCC\tClashCS\tClashSS\tXScore\tname"<<endl;
	for (int i=0;i<nrep;i++)
	{
		name=base+suffix+"_"+toStr(i)+".pdb";

		scoreIndex=xray.scoreIndex[SQR_DIFF];
		sqrOfDiff=scores[i][scoreIndex];
		
		scoreIndex=xray.scoreIndex[RFACTOR];
		r=scores[i][scoreIndex];

		scoreIndex=xray.scoreIndex[PEARSON];
		pearson=scores[i][scoreIndex];

		scoreIndex=xray.scoreIndex[CLASH_CC];
		clashCC=scores[i][scoreIndex];

		scoreIndex=xray.scoreIndex[CLASH_CS];
		clashCS=scores[i][scoreIndex];

		scoreIndex=xray.scoreIndex[CLASH_SS];
		clashSS=scores[i][scoreIndex];

		file <<sqrOfDiff<<"\t"<<r<<"\t"<<pearson<<"\t"<<clashCC<<"\t"<<clashCS<<"\t"<<clashSS<<"\t"<<xscores[i]<<"\t"<<name<<endl;
	}
}
...
void printXRay(Vector &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayParamStruct &params, string xrayOutFile)
{
	...
	calcIntensity(xray);
	if (params.CorrectionFile!="") correctIntensity(xray);
	//if (params.OccupancyCorrectionFile!="" && params.OptimizeOccupancies) correctOccupancyIntensity(xray, degOfFreedom, params);
...
void optimizeAll(Matrix &degOfFreedoms, Vector &score, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	{
		cout <<"Before minimizeRotationTranslationAnalytical"<<endl;
		//for (int j=0;j<3;j++)
		//{
		args.params.RotationWeight=0;
		cout <<"Before minimizeTranslationOnly"<<endl;
		minimizeTranslationOnly(Proteins, xray, expXRay, params, degOfFreedoms[i]);
		cout <<"After minimizeTranslationOnly"<<endl;
...
bool checkForGap(Vector &scores, Real stdDevBest)
{
	...
	{
		cout <<"stdDevBest= "<<stdDevBest<<" dScore= "<<scores[i]-scores[i-1]<<endl;
...
bool checkForConvergence(Vector &scores, Matrix &degOfFreedom, vector<ProteinStruct> &Proteins)
{
	...
	cout <<"stdDev= "<<stdDev<<" stdDevBest= "<<stdDevBest<<endl;
	//if (true)
...
void calcScoresForPermutations(Matrix &real, Matrix &imag, XRayStruct &xray, XRayStruct expXRay, int nprot, Vector &scores, vector< vector<int> > &permutations, Matrix &rmsds, int nuse, XRayParamStruct &params)
{
	...
	while (true)
	{
		...
		if (count%1000000==0)
		{
			...
			cout <<endl;
			gettimeofday(&end, NULL);
			cout <<"permuteTime= "<<calcTimeDiff(start, end)<<endl;
...
void outputDegreesOfFreedomHeader(XRayParamStruct &params)
{
	...
	ofstream file;
	cout <<"In outputDegreesOfFreedomHeader"<<endl;
	OpenFileForWriting(params.DegOfFreedomTrajectoryFile, file);
...
Real calcOrientation(ProteinStruct &Protein1, ProteinStruct &Protein2)
{
	...
	cout <<"natom1= "<<Protein1.Atoms.size()<<endl;
	cout <<"natom2= "<<Protein2.Atoms.size()<<endl;
...
void outputTrajectorySnapShot(Vector &degOfFreedom, vector<ProteinStruct> &Proteins, ProteinStruct &Native, XRayStruct &xray, XRayStruct &expXRay, string name, ofstream &file, XRayParamStruct &params)
{
	...
	setSegmentOccupancies(Proteins[0], degOfFreedom, 1);
	string outputFile;
	outputFile="/home/joukov/project/mr/run/Decoys_CB_2_hours_3Drobot_clash_2_hours_CalphaRadius_4.2/1DVO/pdb_out/"+name+".pdb";
	printReplica(degOfFreedom, Proteins, outputFile, params);
...
void printCorrectConformation(vector<ProteinStruct> Proteins, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	...
	OpenFileForWriting(params.TrajectoryOutputFile, file);
	cout <<"Before readPdb"<<endl;
	readPdb(params.NativePdbFile, Native);
	cout <<"Before calcTMScore"<<endl;
...
void deleteMoves(vector<MoveStruct> &moves)
{
	...
	{
		moves[i].score.clear();
		moves[i].dScore.clear();
		moves[i].temp.clear();
		moves[i].acceptProb.clear();
		moves[i].time.clear();
		moves[i].index.clear();
		moves[i].size.clear();
		moves[i].moveSizeProb.clear();		
	}
}

void cleanMemory(vector<MoveStruct> &moves)
{
	int Size=moves[0].score.size();
	int maxSize=40000;

	if (Size>maxSize) deleteMoves(moves);
}

void sortOnce(Matrix &degOfFreedoms, Vector &score)
{
	int nrep=score.size();
	cout <<"nrep= "<<nrep<<endl;
	cout <<"ndeg= "<<degOfFreedoms.size()<<endl;
	for (int i=1;i<nrep;i++)
	{
		if (score[i]<score[i-1])
		{
			swap(score[i], score[i-1]);
			swap(degOfFreedoms[i], degOfFreedoms[i-1]);
		}
	}
}

void sortReplicasByScore(Matrix &degOfFreedoms, Vector &score)
{
	int nrep=score.size();

	for (int i=0;i<nrep;i++)
	{
		sortOnce(degOfFreedoms, score);
		for (int j=0;j<nrep;j++)
		{
			cout <<"j= "<<j<<" score= "<<score[j]<<endl;
		}
		cout <<endl;
	}
...
Real replicaExchange(Matrix &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params, Real minTemp, Real maxTemp, int nrep, int nsteps, int ncycles)
{
	...
	cout <<"Before initializeMoves"<<endl;
	...
	for (int i=0;i<ncycles;i++)
	{
		...
		if (params.EndIfConverged && i%checkConvergenceInterval==0)
		{
			...
			if (converged) 
			{
				cout <<"Replica exchange converged"<<endl;
				break;
			}
		}
		...
		{
			cout <<"coefficient["<<k<<"]= "<<degOfFreedoms[best][k]<<endl;
		}
		gettimeofday(&end, NULL);
		cout <<"time= "<<calcTimeDiff(start, end)/3600.0
	...
	//multipleSwaps(degOfFreedoms, score, totalSwaps, swapAttempts, temps, hotOrCold, hotHistogram, coldHistogram, 300);
	calcAccurateScores(degOfFreedoms, Proteins, xray, expXRay, scores, score, params);
	calcStandardDeviations(scores, xray.stdDev);
	calcAccurateScores(degOfFreedoms, Proteins, xray, expXRay, scores, score, params);
	sortReplicasByScore(degOfFreedoms, score);
	printSwapRates(totalSwaps, swapAttempts);
	printMoveSizes(moves);
	cout <<"Before printMoveProbabilities"<<endl;
	printMoveProbabilities(moves);
	cout <<"After printMoveProbabilities"<<endl;
	...
	cout <<"Before printScores"<<endl;
	printScoresToFile(degOfFreedoms, Proteins, xray, expXRay, params);
	cout <<"After printScores"<<endl;
	printXRay(degOfFreedoms, Proteins, xray, params);
	for (int i=0;i<Size;i++)
	{
		cout <<"coefficient["<<i<<"]= "<<degOfFreedoms[0][i]<<endl;
	}
	...
	cout <<"bestScore= "<<bestScore<<endl;
	cout <<"Leaving replicaExchange"<<endl;
	return bestScore;
}
...
void initializeDegOfFreedomNoNCS(Matrix &degOfFreedoms, ProteinStruct &Protein, XRayParamStruct &params)
{
	...
	cout <<"Before replicaExchange"<<endl;
	if (params.DegOfFreedomFile!="" && exist(params.DegOfFreedomFile))
	//if (params.DegOfFreedomFile!="")
	{
...
void addOccupanciesToDegOfFreedom(Matrix &degOfFreedoms, XRayParamStruct &params)
{
	...
	if (params.SegmentsFile!="")
	{
		...
		NumOccupancySegments=segmentStart.size()+1;
	}
	else NumOccupancySegments=params.NumOccupancySegments;

	cout <<"NumOccupancySegments= "<<NumOccupancySegments<<endl;
	for (int i=0;i<nrep;i++)
	{
		for (int j=0;j<NumOccupancySegments;j++)
		{
			SafePushBack(degOfFreedoms[i], Real(1.0), "degOfFreedoms");
		}
	}
}
...
void turnOnOptimizeOccupancies(Matrix &degOfFreedoms, vector<ProteinStruct> &Proteins, XRayStruct &xray, XRayParamStruct &params, XRayParamStruct &copyParams)
{
	...
	int nrep;
	int ndof;
	int nPhysicalDof=6*nprot;
	cout <<"In turnOnOptimizeOccupancies"<<endl;
	Get2DVectorSize(degOfFreedoms, nrep, ndof, "degOfFreedoms");
	copyParams.NumOccupancySegments=10;
	copyParams.OptimizeOccupancies=true;
	if (params.SegmentsFile!="")
	{
		calcScatteringContinuousSegmentsFile(Proteins[0], xray, copyParams);	
	}
	else
	{
	...
	//if (params.DegOfFreedomFile=="")
	cout <<"ndof= "<<ndof<<" nPhysicalDof= "<<nPhysicalDof<<endl;
	if (ndof==nPhysicalDof)
	{
		addOccupanciesToDegOfFreedom(degOfFreedoms, copyParams);
	}
}
...
Real MRReplicaExchange(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	cout <<"In MRReplicaExchange"<<endl;
	...
	cout <<"MaxReplicaExchangeTime= "<<params.MaxReplicaExchangeTime<<endl;
	if (params.MaxReplicaExchangeTime>0)
	{
		replicaExchange(degOfFreedoms, Proteins, xray, expXRay, lattice, params, minTemp, maxTemp, nrep, nstep, ncycles);
	}
	if (params.MaxReplicaExchangeClashTime>0)
	{
		copyParams=params;
		copyParams.ClashScoreUse="DuringMR";
		copyParams.MaxReplicaExchangeTime=params.MaxReplicaExchangeClashTime;
		cout <<"Before clash replicaExchange"<<endl;
		replicaExchange(degOfFreedoms, Proteins, xray, expXRay, lattice, copyParams, minTemp, maxTemp, nrep, nstep, ncycles);
		cout <<"After clash replicaExchange"<<endl;
	}
	if (params.MaxReplicaExchangeOptimizeOccupancyTime>0)
	{
		copyParams=params;
		copyParams.ClashScoreUse="DuringMR";
		cout <<"Before turnOnOptimizeOccupancies"<<endl;
		turnOnOptimizeOccupancies(degOfFreedoms, Proteins, xray, params, copyParams);
		cout <<"After turnOnOptimizeOccupancies"<<endl;
		replicaExchange(degOfFreedoms, Proteins, xray, expXRay, lattice, copyParams, minTemp, maxTemp, nrep, nstep, ncycles);
	}
	cout <<"After replicaExchange"<<endl;
	degOfFreedom=degOfFreedoms[0];
	cout <<"Before conjugateGradient"<<endl;
...
Real optimizeBFactorScale(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	...
	cout <<"bFactorScale= "<<bFactorScale[0]<<endl;
	SafePushBack(Proteins, Protein, "Proteins");
	return calcScattering(Proteins, xray, expXRay, params);
...
Real optimizeFormFactorScale(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, XRayParamStruct &params)
{
	...
	cout <<"In optimizeFormFactorScale"<<endl;	
	args.Protein=Protein;
	args.xray=xray;
	args.expXRay=expXRay;
	args.params=params;
	...
	solventCorrectedScatteringFactor(args.xray.f, args.xray.q, args.params);
	cout <<"formFactorScale= "<<formFactorScale[0]<<endl;
	SafePushBack(Proteins, Protein, "Proteins");
	return calcScattering(Proteins, xray, expXRay, params);
...
void molecularReplacement(ProteinStruct &Protein, XRayStruct &xray, XRayStruct &expXRay, LatticeStruct &lattice, XRayParamStruct &params)
{
	...
	cout <<"In molecularReplacment"<<endl;
	SafePushBack(Proteins, Protein, "Proteins");
	...
	cout <<"After MRReplicaExchange"<<endl;
...
void molecularReplacement(string inputPdbFile, XRayParamStruct &params)
{
	...
	cout <<"In molecularReplacement"<<endl;
	parsePdb(inputPdbFile, Protein, params.RemoveWaters, params.RemoveUnknownAtoms);
	cout <<"bfactor= "<<Protein.Atoms[0].BFactor<<" "<<__LINE__<<" "<<__FILE__<<endl;
	cout <<"After parsePdb"<<endl;
	calcUnitCellVectors(Protein, params);
	cout <<"bfactor= "<<Protein.Atoms[0].BFactor<<" "<<__LINE__<<" "<<__FILE__<<endl;
	findCoreAtoms(Protein.Atoms, params.CalphaRadius, params.AsaCutoff);
	cout <<"bfactor= "<<Protein.Atoms[0].BFactor<<" "<<__LINE__<<" "<<__FILE__<<endl;
	cout <<"After calcUnitCellVectors"<<endl;
	...
	cout <<"Before removeNegativeIntensity"<<endl;
	...
	initializeSegments(Protein, params);
	cout <<"bfactor= "<<Protein.Atoms[0].BFactor<<" "<<__LINE__<<" "<<__FILE__<<endl;
	cout <<"Before removeSystematicAbsences"<<endl;
	...
	cout <<"xray.hBinSize= "<<xray.hBinSize<<endl;
	cout <<"xray.scoreIndex.size()= "<<xray.scoreIndex.size()<<endl;
